...
module.exports = class Note {
  static all() {
    const notes = JSON.parse(fs.readFileSync(db_path, { encoding: 'utf-8' }))[
      'notes'
    ];
  ...
  static update(id, data) {
    ...
    const notes = this.all().map(
      note => note.id == id ? Object.assign(note, data) : note
    );
  ...
  static save(notes) {
    fs.writeFileSync(db_path, JSON.stringify({ notes: notes }), {
      encoding: 'utf-8',
    });
    return true;
  }
